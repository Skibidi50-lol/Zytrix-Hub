...
function OwlLib:SetTitle(text)
    local titleLabel = mainFrame.topBarFrame:FindFirstChild("titleLabel") 
        or mainFrame.topBarFrame:FindFirstChildOfClass("TextLabel")
...
for _, v in pairs(mainFrame.topBarFrame:GetChildren()) do
    if v:IsA("ImageLabel") or v:IsA("TextLabel") then
        if v.Name ~= "exitBtn" and v.Name ~= "miniBtn" then
            v:Destroy()
        end
    end
end
...
title.Parent = mainFrame.topBarFrame
